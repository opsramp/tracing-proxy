...
type ClientConfig struct {
	...
	// If it is specified during lib-trace initialization, it will be used as the
...
func NewClient(conf ClientConfig) (*Client, error) {
	...
	if conf.Transmission == nil {
		c.transmission = &transmission.TraceProxy{
			...
			PendingWorkCapacity:  constants.DefaultPendingWorkCapacity,
	...
	if err := c.transmission.Start(); err != nil {
		c.logger.Error().Logf("transmission client failed to start: %s", err.Error())
		return nil, err
	}
...
func (c *Client) Close() {
	c.ensureLogger()
	c.logger.Info().Logf("closing libhoney client")
	if c.transmission != nil {
		_ = c.transmission.Stop()
	}
}
...
func (c *Client) Flush() {
	c.ensureLogger()
	c.logger.Info().Logf("flushing libhoney client")
	if c.transmission != nil {
		if err := c.transmission.Flush(); err != nil {
			c.logger.Error().Logf("unable to flush: %v", err)
		}
	}
}
